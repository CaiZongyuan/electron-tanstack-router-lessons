...
- **没有 Go server、没有 mobile、没有 multica 那套云端 task server / daemon 编排**：这些都是 multica 的范围，本项目一律不涉及。**但会新增一个本地 Node.js daemon 学习支线**，作为 web 与 desktop 共用的本地 agent runtime。不要照搬云端分发相关规则。
- 进度与讲解分两条线放在 `docs/frontend/` 与 `docs/daemon/`：前者讲共享前端，后者讲本地 daemon，分别按 `00 → 01 → 02 …` 推进。
...
包名统一 `@demo/` 前缀。共享包**导出原始 `.ts/.tsx` 源码**（非编译产物），由各 app 的 Vite 各自编译。依赖方向**单向**：`apps → @demo/views → @demo/ui + @demo/core`；`@demo/ui` 与 `@demo/core` 互相独立。`apps/daemon` 只依赖 `@demo/core` 的类型与纯函数，不反向依赖 `@demo/views` 或 `@demo/ui`。
...
- `apps/daemon` 允许使用 Node.js / HTTP / `child_process`，但不允许依赖 DOM、渲染层状态或 UI 组件。
...
6. 与 daemon 的通信接口定义放 `@demo/core`；web 与 desktop 各自注入实现，`@demo/views` 不直接写 IPC 或 localhost HTTP。
...
- ⚠️ **与本地 daemon 的通信也必须抽象**：共享页面只依赖 `@demo/core` 里的 `DaemonClient` 接口；desktop renderer 经 preload IPC 调主进程，由主进程管理 daemon 并转成 HTTP；web renderer 直接用浏览器 `fetch` 连接同一套 localhost HTTP API。不要在 `@demo/views` 里写死 `window.desktopAPI` 或 `http://127.0.0.1:19514`。
...
pnpm dev:daemon         # 起本地 daemon（阶段 4 后按 daemon 文档推进）
pnpm build              # 全量构建
...
| 文档 | 作用 |
|---|---|
| `docs/frontend/README.md` | 前端学习文档索引 |
| `docs/frontend/00-学习计划.md` | web + desktop 共享前端总纲 |
| `docs/daemon/README.md` | 本地 daemon 学习文档索引 |
| `docs/daemon/00-学习计划.md` | daemon 总纲：目标架构、阶段路径、约定 |
...
- **不要在 `@demo/views` 里直调 `window.desktopAPI` 或 daemon HTTP**：走 `@demo/core` 的能力接口 / `DaemonClient`，否则共享页面会与具体壳子耦合，在另一端直接崩。
